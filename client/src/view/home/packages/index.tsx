...
const Packages = () => {
  ...
  const themeClasses = (theme: "primary" | "dim") => {
    if (theme === "primary") {
      return {
        card: "bg-gradient-to-b from-[#102725] via-[#0f2321] to-[#0c1b19] border border-[#1e524d] shadow-[0_0_40px_rgba(105,254,193,0.06)]",
    ...
    return {
      card: "bg-gradient-to-b from-[#0f1620] via-[#0d141c] to-[#0b121a] border border-[#1f2833]",
  ...
  return (
    <section className="pb-10 px-4 relative overflow-hidden">
      <h2 className="text-center text-2xl xl:text-4xl font-bold uppercase">Our Packages</h2>
          ...
          {packagesData.map((pkg) => {
            ...
            return (
              <div key={pkg.id} className={`relative ${isPrimary ? "md:-mt-6" : ""}`}>
                ...
                >
                  {/* side dashed rails */}
                  <span className={`absolute top-20 bottom-24 left-4 border-l border-dashed ${t.side} opacity-60`}></span>
                  <span className={`absolute top-20 bottom-24 right-4 border-l border-dashed ${t.side} opacity-60`}></span>
                  {/* little tabs */}
                  ...
                  <div className="text-center mb-6">
                    <div className={`text-5xl font-extrabold tracking-tight ${t.price}`}>
                      {pkg.price}
                      <span className="text-xs lg:text-sm text-[#b0bac8] font-normal"> / Only</span>
                    </div>
                  </div>
                  ...
                  <div className="">
                    <button
                      className={`${t.button} w-full py-3 rounded-md font-semibold tracking-wide hover:brightness-110 transition`}
                    >
                      CHOOSE PLAN
                    </button>
                  </div>
